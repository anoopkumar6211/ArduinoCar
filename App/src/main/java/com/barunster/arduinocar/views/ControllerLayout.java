 ...
 */
public class ControllerLayout extends RelativeLayout implements View.OnLongClickListener{
    ...
    public ControllerLayout(Context context, int rows, int columns) {
        super(context);
...
//                        Log.d(TAG," Hiding in pos = " + i);
                    ...
                    ((DropZoneFrame)this.getChildAt(i)).setParentId(frame.getId());
                    ((DropZoneFrame)this.getChildAt(i)).setEmpty(false);
                }
            }
...
//                    Log.d(TAG, "Button orientation is vertical");
                ...
                // Hiding not relevant frames
                for (int i = btn.getPosition() + columnsAmount ; i < btn.getPosition() + (btn.getSize()*columnsAmount) ; i+=columnsAmount)
                {
                    Log.d(TAG," Hiding in pos = " + i);
                    this.getChildAt(i).setVisibility(View.GONE);
                    ((DropZoneFrame)this.getChildAt(i)).setParentId(frame.getId());
    ...
    public boolean onLongClick(View v) {
        ...
        if (editing)
        {
            ...
            else if (v instanceof SlideButtonLayout)
            {
                ...
                if (((SlideButtonLayout) v).getType() == CustomButton.BUTTON_TYPE_SLIDE_HORIZONTAL)
                {
                    dropZoneImage.setOrientation(LinearLayout.HORIZONTAL);
                    for (int i = frame.getId(); i < ( frame.getId() + buttonSize) ; i++)
                    {
                ...
                {
                    dropZoneImage.setOrientation(LinearLayout.VERTICAL);
                    for (int i = frame.getId(); i < frame.getId() + (buttonSize * columnsAmount) ;  i+= columnsAmount)
                    {
                        Log.d(TAG," Showing in pos = " + i);
                        this.getChildAt(i).setVisibility(View.VISIBLE);
                        ((DropZoneFrame)this.findViewById(i)).setEmpty(true);
...
//                                Log.d(TAG, "DropZone Image doesnt have a tag");
        ...
        private void calcMovementOptions(DropZoneFrame frame){
...
*/
            ...
            do
            {
                ...
                canGoRight = canGoRight && // Making sure the last check was ok to
                        ...
                        ControllerLayout.this.getChildAt(frame.getId() + count) != null && //  Check that the view isnt null
                        ((DropZoneFrame) ControllerLayout.this.getChildAt(frame.getId() + count)).isEmpty();
            ...
            processMovementOptionFindings(frame, canGoLeft, canGoRight, leftSteps, rightSteps);
        }

        private void processMovementOptionFindings(DropZoneFrame frame, boolean canGoLeft, boolean canGoRight, int leftSteps, int rightSteps){
            int count;
...
//                                Log.d(TAG, "There is a place with right and left combine");
                     ...
                     i += dropZoneImage.getOrientation() == LinearLayout.HORIZONTAL ? 1 : columnsAmount/*?rowsAmount*/)
                {
                    ...
                    ((DropZoneFrame) ControllerLayout.this.getChildAt(i)).setToAvailableMode();
                    highlightedZones.add(((DropZoneFrame) ControllerLayout.this.getChildAt(i)));
                }
            }
      /*                      else if (rightSteps + leftSteps + 1 > dropZoneImage.getSize()) // TODO May cause some problems
            ...
            else if (canGoLeft || canGoRight)
            {
                ...
                while (count < dropZoneImage.getSize() * (dropZoneImage.getOrientation() == LinearLayout.HORIZONTAL ? 1 : columnsAmount/*?rowsAmount*/) )
                {
                    if (canGoLeft)
                    {
                        ((DropZoneFrame) ControllerLayout.this.getChildAt(frame.getId() - count)).setToAvailableMode();
                        highlightedZones.add(((DropZoneFrame) ControllerLayout.this.getChildAt(frame.getId() - count)));
                    }
                    else
                    {
                        ((DropZoneFrame) ControllerLayout.this.getChildAt(frame.getId() + count)).setToAvailableMode();
